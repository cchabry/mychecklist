...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return <Card className="bg-[#fff8f0] backdrop-blur-md rounded-lg border border-tmw-teal/20 shadow-md transition-shadow hover:shadow-lg">
      <CardHeader className="relative">
        <Link to={`/project/edit/${project.id}`} className="absolute top-4 right-4 p-1.5 text-gray-500 hover:text-tmw-teal transition-colors">
          <Pencil size={16} />
        </Link>
            ...
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Mis à jour {formatDistanceToNow(new Date(audit.updatedAt), {
                addSuffix: true,
                locale: fr
              })}
                </p>
